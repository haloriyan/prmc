...
import { BiCheck, BiEdit, BiTrash, BiUpload, BiX } from "react-icons/bi";
...
const GigsEdit = () => {
    ...
    const [buttonText, setButtonText] = useState(translation.general.edit[lang]);
    ...
    useEffect(() => {
        if (isLoading) {
            ...
            .then(response => {
                ...
                let gig = res.gig;
                let coverSource = `${config.baseUrl}/storage/service_images/${gig.cover}`;
                setTitle(gig.title);
                setDescription(gig.description);
                setAddress(gig.address);
                setCover(coverSource);

                const thePakets = gig.packages.map(pkg => ({
                    ...pkg,
    ...
    useEffect(() => {
        if (cover !== null && typeof cover === 'string' && isLink(cover) && coverRef.current) {
            coverRef.current.src = cover;
        }
    }, [coverRef, cover]);
    ...
    const submit = () => {
        ...
        axios.post(`${config.baseUrl}/api/service/${id}/update`, formData)
        .then(response =>{
            navigate(-1);
        })
    }
        ...
        <>
            ...
            <div className="absolute top-16 right-0 w-9/12 mobile:w-full py-12 pe-8 mobile:p-8 flex flex-col">
                <div className="flex items-center gap-4">
                    ...
                    <div className="text-slate-700 font-bold text-xl">{translation.general.edit[lang]} Gigs</div>
                </div>
                ...
                <div className="flex gap-4 mt-8">
                    <div className="w-6/12">
                        {
                            ...
                            <>
                                ...
                                <div className="flex justify-end mt-2">
                                    <div className="flex items-center gap-2 cursor-pointer" onClick={() => setCover(null)}>
                                        <BiEdit />
                                        <div className="text-sm">{translation.general.edit[lang]} {translation.general.image[lang]}</div>
                                    </div>
                                </div>
                            </>
                        }
                    ...
                    <div className="flex flex-col gap-4 grow">
                        ...
                        <button className="w-full h-10 border border-primary text-primary text-sm" onClick={() => setAddingPaket(true)}>
                            {translation.general.edit[lang]} {translation.services.package[lang]}
                        </button>
                    </div>
                </div>

                <button className="fixed bottom-10 right-10 p-3 px-6 font-bold bg-primary text-white" onClick={submit}>
                    {translation.general.save_changes[lang]}
                </button>
            </div>
                ...
                <Popup onDismiss={() => setAddingPaket(false)}>
                    ...
                    <form onSubmit={savePaket}>
                        ...
                        <div className="text-right text-primary text-sm cursor-pointer" onClick={() => {
                            ...
                            bens.push('');
                            setPaketBenefits(bens);
                        }}>{translation.general.edit[lang]} Benefit</div>

                        <button className="w-full bg-primary h-10 text-sm text-white" onClick={savePaket}>{translation.general.edit[lang]}</button>
                    </form>
                </Popup>
            }
        </>
